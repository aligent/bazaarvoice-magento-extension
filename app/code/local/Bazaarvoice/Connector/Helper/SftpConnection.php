...
class Bazaarvoice_Connector_Helper_SftpConnection extends Mage_Core_Helper_Abstract
{
    ...
    public function connect($host, $port, $user, $pass)
    {
        try {
            ...
            // Log
            Mage::log('     BVSFTP - Host: ' . $server, Zend_Log::DEBUG, Bazaarvoice_Connector_Helper_Data::LOG_FILE);
            Mage::log('     BVSFTP - Username: ' . $username, Zend_Log::DEBUG, Bazaarvoice_Connector_Helper_Data::LOG_FILE);
//            Mage::log('Password: ' . $password, Zend_Log::DEBUG, Bazaarvoice_Connector_Helper_Data::LOG_FILE);
    ...
    {
        Mage::log("     BVSFTP - Get remote file $remoteFilePath => $localFilePath", Zend_Log::DEBUG, Bazaarvoice_Connector_Helper_Data::LOG_FILE);
        try {
            // Close connection
            if (!$this->isConnected()) {
                return false;
    ...
    public function putFile($localFilePath, $remoteFile = '')
    {
        ...
        Mage::log("     BVSFTP - Put local file $localFilePath as $remoteFile", Zend_Log::DEBUG, Bazaarvoice_Connector_Helper_Data::LOG_FILE);
        try {
            // Close connection
            if (!$this->isConnected()) {
                return false;
